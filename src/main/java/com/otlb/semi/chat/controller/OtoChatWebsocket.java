...
//		System.out.println("[OtoChatWebsocket][userProp]"+userProp); 
		...
		if (sndSess == null || rcvSess == null) {
			System.out.println("[OtoChatWebsocket][onMessage1][대화창모두 종료]" + sndSess + ":" + senderId + ":" + receiverId
					+ ":" + rcvSess);
			return;
		}
		try {
			...
			System.out
					.println("[OtoChatWebsocket][채팅불가3]" + sndSess + ":" + senderId + ":" + receiverId + ":" + rcvSess);
	...
	public void onClose(Session session) throws IOException {
		...
		// 상대방들에게 모두 메세지를 출력한다
		System.out.println("[OtoChatWebsocket][대화창 종료시 상대방]" + userId + ">>" + thisPartyStr);
		Map<String, Session> clients = OtoChatInfo.getOtoChatClients();
		Collection<Session> sessionsList = clients.values();
		...
		for (Session thisSess : sessionsList) {
			...
			if (thisPartyStr.indexOf(thisSessUserId) != -1) {
				// 상대방의 파티에서 제외한다
				String checkPartyStr = partyMap.get(thisSessUserId);
				System.out.println("[OtoChatWebsocket][대화창 종료시 상대방]파티제외전>>" + checkPartyStr + ":" + thisSessUserId);

				checkPartyStr = checkPartyStr.replaceAll(userId, "");
				partyMap.put(thisSessUserId, checkPartyStr);
				System.out.println("[OtoChatWebsocket][대화창 종료시 상대방]파티제외후>>" + partyMap.get(thisSessUserId));
